...
export function WorkoutForm({ prefill }: WorkoutFormProps) {
  const { formData, isSubmitting, submitError, submitSuccess, isFormValid, updateSet, submitWorkout, clearDraft } =
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <form onSubmit={handleSubmit} className="pb-32">
        <div className="container mx-auto px-4 py-6 max-w-2xl">
          {formData.exercises.map((exercise, idx) => (
            <ExerciseInput key={exercise.exercise_id} exercise={exercise} exerciseIndex={idx} updateSet={updateSet} />
          ))}
        </div>
      </form>
      ...
      {submitSuccess && (
        <SuccessToast workoutId={submitSuccess.id} personalBests={submitSuccess.personal_bests_updated} />
      )}
      ...
      {showCancelDialog && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md mx-4">
            ...
            <p className="text-gray-700 mb-6">Are you sure you want to cancel? Your progress will be lost.</p>
