...
export function ExerciseSelector({ onSelect, selectedExerciseIds, disabled = false }: ExerciseSelectorProps) {
  const [open, setOpen] = React.useState(false);
  const [search, setSearch] = React.useState("");
  const { exercises, loading, error } = useExercises({ search });

  const availableExercises = exercises.filter((exercise) => !selectedExerciseIds.includes(exercise.id));
  ...
  React.useEffect(() => {
    ...
    console.log(
      "First 3 exercises:",
      availableExercises.slice(0, 3).map((e) => ({ id: e.id, name: e.name }))
    );
    ...
    <div className="space-y-2" data-test-id="exercise-selector-container">
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            <span className="truncate">{loading ? "Loading exercises..." : "Select an exercise"}</span>
                  ...
                  {availableExercises.map((exercise) => (
                    ...
                    >
                      <Check
                        className={cn(
                          "mr-2 h-4 w-4",
                          selectedExerciseIds.includes(exercise.id) ? "opacity-100" : "opacity-0"
                        )}
                      />
                      {exercise.name}
                    </button>
