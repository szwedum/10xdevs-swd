...
export function TemplateNameInput({ value, onChange, onBlur, error, autoFocus = false }: TemplateNameInputProps) {
  return (
    <div className="space-y-2" data-test-id="template-name-container">
      ...
      />
      {error && (
        <p id="template-name-error" className="text-sm text-destructive" aria-live="polite">
          {error}
        </p>
      )}
    </div>
