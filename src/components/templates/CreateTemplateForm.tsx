...
import {
  ...
  validateWeight,
  validateForm,
} from "@/lib/validation/template-form.validation";
...
function CreateTemplateFormInner() {
  ...
  const { createTemplate, error: submitError } = useCreateTemplate();
  ...
  const handleExerciseUpdate = (id: string, field: "sets" | "reps" | "default_weight", value: number) => {
    setFormState((prev) => ({
      ...prev,
  ...
  return (
    <form
      ...
      className="space-y-8"
    >
      <TemplateNameInput value={formState.name} onChange={handleNameChange} error={formState.nameError} autoFocus />
...
export default function CreateTemplateForm({
  ...
  console.log("CreateTemplateForm - props:", {
    supabaseUrl,
